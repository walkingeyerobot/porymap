...
QPixmap Map::renderCollision(qreal opacity, bool ignoreCache) {
    ...
    for (int i = 0; i < layout->blockdata->blocks->length(); i++) {
        ...
        changed_any = true;
        Block block = layout->blockdata->blocks->value(i);
        QImage metatile_image = getMetatileImage(block.tile, layout->tileset_primary, layout->tileset_secondary, projectConfig.getTripleLayerMetatilesEnabled());
...
QPixmap Map::renderBorder(bool ignoreCache) {
    ...
    for (int i = 0; i < layout->border->blocks->length(); i++) {
        ...
        QImage metatile_image = getMetatileImage(tile, layout->tileset_primary, layout->tileset_secondary, projectConfig.getTripleLayerMetatilesEnabled());
        int map_y = width_ ? i / width_ : 0;
        int map_x = width_ ? i % width_ : 0;
